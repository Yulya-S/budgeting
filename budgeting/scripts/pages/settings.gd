...
func _ready() -> void:
	...
	for i in get_children():
		match i.get_class():
			"CheckButton":
				i.button_pressed = bool(data[SF.l(i)])
				call("_on_"+SF.l(i)+"_toggled", i.button_pressed)
			"OptionButton":
				call("_on_"+str("cus" if data.color_preset else "pre"), data.color_scheme)
			"Control":
...
func _on_event_page_calendar_toggled(_toggled_on: bool) -> void: File.set_CB(EventType)
