...
func _draw() -> void:
	...
	# Отрисовка
	for i in range(day_count): draw_string(ThemeDB.fallback_font, Vector2(x_step*i, 120), str(i+1.), HORIZONTAL_ALIGNMENT_CENTER, x_step, 9, ColorScheme.get_color(0, 1, ColorScheme.system_gradient))
	for i in values:
		...
		draw_rect(Rect2(Vector2(x_step*(int(i.day)-1) - 2, 60 - data[1] - 2), Vector2(x_step + 4, y_size + 4)), ColorScheme.get_color(0, 1, ColorScheme.system_gradient))
		draw_rect(Rect2(Vector2(x_step*(int(i.day)-1), 60 - data[1]), Vector2(x_step, y_size)), data[0])
		draw_string(ThemeDB.fallback_font, Vector2(x_step*(int(i.day)-1.), data[2]), str(i.value), HORIZONTAL_ALIGNMENT_CENTER, x_step, 9, ColorScheme.get_color(0, 1, ColorScheme.system_gradient))
